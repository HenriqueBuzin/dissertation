...
print(f"HTTP_SERVER_URL: {HTTP_SERVER_URL}")
...
def send_data_http(data, http_url):
    ...
    try:
        ...
        print(f"Dados enviados: {data}, Resposta do servidor: {response.status_code}, {response.text}")
    except Exception as e:
...
if __name__ == '__main__':
    if not HTTP_SERVER_URL or not CSV_URL or not INSTANCE_DATA:
        print("Erro: As variáveis de ambiente 'CSV_URL', 'HTTP_SERVER_URL', e 'INSTANCE_DATA' devem ser definidas.")
        sys.exit(1)
