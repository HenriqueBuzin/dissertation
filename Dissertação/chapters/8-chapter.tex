...
Os experimentos realizados com medidores de energia e água demonstraram que a arquitetura proposta mantém um fluxo de dados consistente desde a borda até a nuvem. A abstração de protocolos, aplicada entre HTTP e CoAP, simplificou a interoperabilidade entre dispositivos distintos, enquanto o uso da plataforma \gls{HPCC} \textit{Systems} na camada de nuvem possibilitou a agregação e a análise dos dados em larga escala.
...
Como perspectivas futuras, recomenda-se a integração de novos protocolos de comunicação, como \textit{MQTT}, a adoção de mecanismos de autenticação e a criptografia ponta a ponta.
