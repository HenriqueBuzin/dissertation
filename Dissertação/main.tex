...
\titulo{IMSA: Uma Arquitetura de Software Modular para Ambientes Computacionais de Névoa}
...
\orientador[Orientadora]{Prof.ª Dr.ª Patricia Della Méa Plentz}
% FIXME Substituir 'XXXXXX' pelo nome do seu
...
\siglalista{CAPES}{Coordenação de Aperfeiçoamento de Pessoal de Nível Superior}
\siglalista{UFSC}{Universidade Federal de Santa Catarina}

% Área de atuação
\siglalista{TI}{Tecnologia da Informação}

% Computação em nuvem e modelos de serviço
\siglalista{IaaS}{Infraestrutura como Serviço}
\siglalista{PaaS}{Plataforma como Serviço}
\siglalista{SaaS}{Software como Serviço}
\siglalista{AWS}{Amazon Web Services}

% Computação em névoa / IoT
\siglalista{IoT}{Internet of Things}
\siglalista{RTT}{Round Trip Time}
\siglalista{RMSE}{Root Mean Square Error}

% Protocolos e tecnologias de comunicação
\siglalista{TCP/IP}{Transmission Control Protocol / Internet Protocol}
\siglalista{TCP}{Transmission Control Protocol}
\siglalista{HTTP}{Hypertext Transfer Protocol}
\siglalista{CoAP}{Constrained Application Protocol}
\siglalista{SFTP}{Secure File Transfer Protocol}
\siglalista{FTP}{File Transfer Protocol}
\siglalista{SSH}{Secure Shell}
...
\siglalista{MQTT}{Message Queuing Telemetry Transport}
\siglalista{AMQP}{Advanced Message Queuing Protocol}
\siglalista{REST}{Representational State Transfer}
\siglalista{L2}{Layer 2 Fog}
\siglalista{L1}{Layer 1 Fog}

% Plataforma de processamento de dados
\siglalista{HPCC}{High Performance Computing Cluster}
\siglalista{HPC}{High Performance Computing}
\siglalista{ECL}{Enterprise Control Language}
\siglalista{GRAPHQL}{Graph Query Language}
\siglalista{ROXIE}{Rapid Online XML Inquiry Engine}
\siglalista{IMSA}{Inter-fog Modular Software Architecture}

% Aprendizado de máquina citado na revisão
\siglalista{LSTM}{Long Short-Term Memory}

% Hardware (aparece na tabela de ambiente de testes)
\siglalista{CPU}{Central Processing Unit}
\siglalista{RAM}{Random Access Memory}
\siglalista{SSD}{Solid-State Drive}
...
% Banco de dados (IMPORTANTE pro 2.4)
\siglalista{SGBD}{Sistema de Gerenciamento de Banco de Dados}
\siglalista{SQL}{Structured Query Language}
\siglalista{NoSQL}{Not Only SQL}
